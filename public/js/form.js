...
function active_element($nav = "next") {
	...
	if ($(".footer-progress ul li.active").hasClass("step_6")) {
		$(
			"." +
				$tech_type_val +
				'-container [data-formid="step_no_6"] .project_number'
		).val($(".footer-progress ul li.active").attr("data-divide"));
	} else {
		$(
			"." +
				$tech_type_val +
...
function step_text() {
	...
	$("#steptext").text(
		$remaining_el + " of " + $total_length + " Steps Remaining"
	);
}
	...
	if ($current_step >= 3 && $current_step <= 6) {
		...
		$(
			"." +
				...
				"-container [data-formid='step_no_" +
				$current_step +
				"']"
		).show();
	} else {
...
function card_display() {
	...
	$(
		"." +
			$tech_type_val +
			'-container [data-formid="step_no_6"] #pageinfo .card-project'
	).hide();
	var $current_step_eq =
		parseInt(
			$(".footer-progress ul li.step_6.active").attr("data-divide")
		) - 1;
	$(
		"." +
			$tech_type_val +
			'-container [data-formid="step_no_6"] #pageinfo .card-project:eq(' +
			$current_step_eq +
			")"
	).show();
}
...
function step_btn_display() {
	...
	var $highest_step_number = parseInt(
		$(
			".footer-progress ul li.step_6[data-project-type='" +
				$tech_type_val +
				"']:last"
		).attr("data-divide")
	);
	if (
		$active_selector.hasClass("step_6") &&
		parseInt($active_selector.attr("data-divide")) < $highest_step_number
	) {
	...
	if (
		$active_selector.hasClass("step_6") &&
		parseInt($active_selector.attr("data-divide")) > 1
	) {
...
function toggle_both_functionality() {
	var $tech_type_val = $("#tech_type").val();
	var $type_val = $(
		"." + $tech_type_val + '-container [name="type"]:checked'
	).val();
	var $smf_val = $(
		"." +
			$tech_type_val +
			'-container [name="both_same_functionality"]:checked'
	).val();

	if ($type_val == "both" && $smf_val == "1") {
		$("#tech_type").val("both");
		$("#clientproject")
			.addClass("both-project")
			.removeClass("mobile-project")
			.removeClass("web-project");
		$("#top")
			.addClass("both-project")
			.removeClass("mobile-project")
			.removeClass("web-project");
		...
		$("#tech_type").val("web");
		$("#clientproject")
			.addClass("web-project")
			.removeClass("mobile-project")
			.removeClass("both-project");
		$("#top")
			.addClass("web-project")
			.removeClass("mobile-project")
			.removeClass("both-project");
	...
	if ($type_val == "both") {
		...
		$(
			'.same-functionality-wrap [name="both_same_functionality"][value="1"]'
		).prop("checked", true);
	}
}
...
function data_changed($parent_selector, $type = 1) {
	if ($parent_selector.find(".duplicate").length > 0) {
		var $duplicate_checked = $parent_selector
			.find(".duplicate")
			.prop("checked");
...
$(function () {
	...
	$("body").on("change", '[name="type"]', function () {
		...
		$("#clientproject .project-type-container").each(function () {
			$(this)
				.find(
					'[data-formid="step_no_2"] [name="type"][value="' +
						$this_val +
						'"]'
				)
				.prop("checked", true);
	...
	$("body").on("change", '[name="both_same_functionality"]', function () {
		...
		$("#clientproject .project-type-container").each(function () {
			$(this)
				.find(
					'[data-formid="step_no_2"] [name="both_same_functionality"][value="' +
						$this_val +
						'"]'
				)
				.prop("checked", true);
	...
	$("body").on("click", ".duplicate", function () {
		...
		//$(this).closest('.card-project').find('.duplicate-data-changed').val('0');
		if ($(this).is(":checked")) {
			var $copyfrom_cardid = parseInt(
				$(this).closest(".card-project").attr("data-number")
			);
			var $copyto_cardid =
				parseInt($(this).closest(".card-project").attr("data-number")) +
				1;

			$(
				"." +
					$tech_type_val +
					"-container [data-formid='step_no_6'] .card-project[data-number='" +
					$copyto_cardid +
					"'] .pinfowrap .values-wrap"
			).html("");
			$number_count = 1;
			$(
				"." +
					...
					"-container [data-formid='step_no_6'] .card-project[data-number='" +
					$copyfrom_cardid +
					"'] .pinfowrap .values-wrap .values-list"
				...
				$(
					"." +
						...
						$copyto_cardid +
						"'] .pinfowrap .values-wrap"
				).append($(this).prop("outerHTML"));

				$copyfrom_user = $(this)
					.closest(".card")
					.find(".card-header .page-user")
					.text();
				$(
					"." +
						$tech_type_val +
						"-container [data-formid='step_no_6'] .card-project[data-number='" +
						$copyto_cardid +
						"'] .pinfowrap .values-wrap textarea"
					...
					var $copy_from_val = $(
						"." +
							...
							"-container [data-formid='step_no_6'] .card-project[data-number='" +
							$copyfrom_cardid +
							"'] .pinfowrap .values-wrap .values-list:eq(" +
							index2 +
							") textarea"
					).val();
			...
			$(
				"." +
					...
					"-container [data-formid='step_no_6'] .card-project[data-number='" +
					$copyfrom_cardid +
					"'] .actwrap .values-wrap .values-list"
			).each(function (index, el) {
				$(
					"." +
						$tech_type_val +
				...
				$(
					...
					var $copy_from_val = $(
						"." +
							...
							"-container [data-formid='step_no_6'] .card-project[data-number='" +
							$copyfrom_cardid +
							"'] .actwrap .values-wrap .values-list:eq(" +
							index2 +
							") textarea"
					).val();
					$(this).val($copy_from_val).addClass("duplicated");
			...
			$(
				"." +
					...
					"-container [data-formid='step_no_6'] .card-project[data-number='" +
					$copyfrom_cardid +
					"'] .pinfowrap .values-wrap .values-list:eq(0)"
			).each(function (index, el) {
				$(
					"." +
						$tech_type_val +
			...
			$(
				"." +
					...
					"-container [data-formid='step_no_6'] .card-project[data-number='" +
					$copyfrom_cardid +
					"'] .actwrap .values-wrap .values-list:eq(0)"
			).each(function (index, el) {
				$(
					"." +
						$tech_type_val +
	...
	$("#addmoreexample").click(function () {
		...
		clone
			.find(".action-btn")
			.removeClass("d-none")
			.attr("data-id", "shmain_" + newel);
		clone.find("input").val("");
		clone
			.find("#example_projectsInput")
			.attr("id", "example_projects_" + newel + "_Input");
		clone
			.find("#example_projectsError")
			.attr("id", "example_projects_" + newel + "_Error");
	...
	$("#backbutton").click(function () {
		...
		if (step != 0) {
			...
			var $tgt_selector = $(
				".mobile-container [data-formid='step_no_2']"
			);
			var $tgt_selector2 = $(".footer-progress li.active");
			if (
				$tgt_selector.find("[name='type']:checked").attr("value") ==
					"both" &&
				$tgt_selector
					.find("[name='both_same_functionality']:checked")
					.attr("value") == "0" &&
				$tgt_selector2.hasClass("step_3") &&
				$tgt_selector2.attr("data-project-type") == "web"
			) {
				$("#tech_type").val("mobile");
				$("#clientproject")
					.addClass("mobile-project")
				...
				if (step == 3) {
					$("#clientproject .project-type-container").each(
						function () {
	...
	$("#submitbtn").click(function () {
		...
		if(typeof loginStatus !== 'undefined' && !loginStatus && formID == 7){
			$('#modalBtn').click();
			signUp();
			modalStatus = true;
		}
		...
		if (formID == "2") {
			...
			if ($type_val == "both" && $smf_val == "0") {
				...
				function myFunction($type_val, index, array) {
					$append_str =
						$append_str +
						'<li class="step_3" data-project-type="' +
						$type_val +
						'"><span class="percentage">42%</span> <span class="dots"></span></li>';
					$append_str =
						$append_str +
						'<li class="step_4" data-project-type="' +
						$type_val +
						'"><span class="percentage">56%</span> <span class="dots"></span></li>';
					$append_str =
						$append_str +
						'<li class="step_5" data-project-type="' +
						$type_val +
						'"><span class="percentage">70%</span> <span class="dots"></span></li>';
					$append_str =
						$append_str +
						'<li class="step_6" data-project-type="' +
						$type_val +
						'" data-divide="1"><span class="percentage">84%</span> <span class="dots"></span></li>';
				}
		...
		if (formID == "5") {
			...
			var $divide_steps = $(
				"#clientproject ." +
					$tech_type_val +
					"-container [class*='checkme_']:checked"
			).length;
			...
			for ($i = 1; $i <= $divide_steps; $i++) {
				$ap_str =
					...
					'" data-divide="' +
					$i +
					'"><span class="percentage">84%</span> <span class="dots"></span></li>';
			}
			$('li.step_5[data-project-type="' + $tech_type_val + '"]').after(
				$ap_str
			);
	...
	$("#submitbtn_projects").click(function () {
		var $active_tab = $(".footer-progress ul li.active").attr(
			"data-divide"
		);
		var $tech_type_val = $("#tech_type").val();
		var $parent_selector = $(
		...
		var $info_val = $parent_selector
			.find(".values-wrap .informationfield:eq(0) textarea")
			.val();
		var $action_val = $parent_selector
			.find(".values-wrap .perfomableinput:eq(0) textarea")
			.val();
		//var $mockup_val = $parent_selector.find(".mockupinput input[type='text']").val();

		if ($info_val == "") {
			$parent_selector
				.find(".values-wrap .informationfield:eq(0)")
				.closest(".values-wrap")
		...
		if ($action_val == "") {
			$parent_selector
				.find(".values-wrap .perfomableinput:eq(0)")
				.closest(".values-wrap")
				.next(".invalid-feedback")
				.text("This field is required");
			$has_errors = true;
    ...
    $(".invalid-feedback."+myid).before(clone);*/
		$(this)
			.closest(".values-component")
			.find(".values-wrap")
			.append(
				$(this)
					.closest(".values-component")
					.find(".values-wrap .values-list:eq(0)")
					.prop("outerHTML")
			);

		$(this)
			.closest(".values-component")
			.find(".values-wrap .values-list:last textarea")
			.val("");
	...
	$("#savelater").click(function () {
		...
		var $tech_type_val = $("#tech_type").val();
		$('[data-formid="step_no_' + step + '"] input[name="is_draft"]').attr(
			"value",
			"1"
		);
		$(
				...
				if (response.status == 200) {
					if (response.url) {
						if (response.step != "draft") {
							var registerBtn = $(
								"#questionnaire-completed-modal"
							).find("#registerBtn");
							if (registerBtn.length) {
								registerBtn.attr("href", response.url);
							}
							$("#questionnaire-completed-modal")
								.addClass("visible")
								.show();
						...
						if (response.permissionHtml) {
							$(
								"." + $tech_type_val + "-container #permission"
							).html(response.permissionHtml);
						}
						if (response.infoHtml) {
							$(
								"." +
									$tech_type_val +
									'-container [data-formid="step_no_6"] #pageinfo'
							).html(response.infoHtml);
						}
						if (response.project_type) {
							$(
								"." +
									$tech_type_val +
									'-container [data-formid="step_no_7"] #project_type_7'
							).val(response.project_type);
							if (response.project_type == "mobile") {
								$(
									"." +
										$tech_type_val +
										'-container [data-formid="step_no_7"] .techlist'
								).hide();
								$(
									"." +
										$tech_type_val +
										'-container [data-formid="step_no_7"] #mobilelist'
								).show();
								...
								$(
									"." + $tech_type_val + "-container #weblist"
								).show();
							} else {
								$(
									"." +
						...
						if (
							e.target.getAttribute("data-formid") == "step_no_1"
						) {
							$("#clientproject .project-type-container").each(
								function () {
									$(this)
										.find('[data-formid="step_no_2"]')
										.show();
								}
							);
						}
...
$(".calling-icon-box").click(displayModal);

function displayModal() {
	showModal($("#calling-modal"));
	$("#callingIconOutside").css("visibility", "hidden");
} 
...
$(".radio-row").click(function () {
	let radiosGroup = $(this).parent();
	radiosGroup.find("input[type=radio]").prop("checked", false);
	$(this).find("input[type=radio]").prop("checked", true);
});

// $(".duration-btn").click(function () {
// 	let curMin = parseInt($("#minCount").html());
// 	if ($(this).hasClass("dr-minus")) {
// 		if (curMin == 10) return;
// 		else $("#minCount").html(curMin - 10);
// 	} else {
// 		if (curMin == 60) return;
// 		else $("#minCount").html(curMin + 10);
// 	}
...
function showJoinMeeting() {
	...
	$("#joinMeeting").removeClass("d-none");
	// var meetingDuration = $( "#meetingDuration option:selected" ).val()
	// console.log(meetingDuration);


	// $(".pay-amount").text("$" + meetingDuration + '.00');
	// $(".pay-minutes").text(meetingDuration);
}

function showMeetingPayment() {
	console.log($("#duration_select").val());
	console.log($("#remain_time").val());
	if( $("#duration_select").val() > $("#remain_time").val()){
		$("#joinMeeting").addClass("d-none");
		$("#meetingPayment").removeClass("d-none");
...
function showPayment() {
	$("#meetingPayment").addClass("d-none");
	$("#paymentFailed").addClass("d-none");
...
function showExtendPayment() {
	$("#extendPaymentFailed").addClass("d-none");
	$("#extendPayment").removeClass("d-none");
}

function showExtendPaymentLoading() {
	$("#extendPayment").addClass("d-none");
	$("#extendPaymentLoading").removeClass("d-none");
}

function showExtendPaymentSuccess() {
	$("#extendPaymentLoading").addClass("d-none");
	$("#extendPaymentSuccess").removeClass("d-none");
}

function showExtendPaymentFailed() {
	$("#extendPaymentLoading").addClass("d-none");
	$("#extendPaymentFailed").removeClass("d-none");
}
...
function showCallOngoingDev() {
	$("#joinCall").addClass("d-none");
	$("#callOngoingDev").removeClass("d-none");
	$("#callersDevSlider").slick("refresh");
}

function showCallOngoing() {
	$("#joinCall").addClass("d-none");
...
function showExtendDuration() {
	$("#extendDuration").removeClass("d-none");
	$("#calling-modal").addClass("extend-duration");
}

function showMeetingEnded() {
	$("#callOngoing").addClass("d-none");
	$("#extendDuration").addClass("d-none");
	$("#meetingEnded").removeClass("d-none");
	$("#calling-modal").removeClass("ongoing-call extend-duration");
}
