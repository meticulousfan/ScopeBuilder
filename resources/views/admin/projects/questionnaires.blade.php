            ...
            <div class="header-inner">
                <div class="header-block with-menu-opener">
                    ...
                    <span style="display: flex;align-items: center;">
                        <h1 class="page-caption" style="margin: 0px!important;">Questionnaires</h1>
                        <div
                            ...
                            <div style="font-size:13px;color:white;">{{ $questionnairesNum }}</div>
                        </div>
                    </span>
                </div>
            </div>
        ...
        <div class="page-content">
            <section class="default-section">
                <div class="container">
                    ...
                    @endif
                    <div>
                        <a href="{{route('admin.questions.create')}}"  class="btn setting-btn" style="float: right;">New Questionnaire</a>
                        {{-- <button class="btn setting-btn" type="button" style="float: right;" data-modal="#new-questionnaire-modal">New Questionnaire</button> --}}
                    </div>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($questionnaires as $questionnaire)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                {{ $questionnaire->project_type_name }}
                                            </td>
                                            <td>
                                                {{ $questionnaire->stepe }}
                                            </td>
                                            <td>{{ date('F jS, Y', strtotime($questionnaire->created_at)) }}</td>
                                            <td>{{ date('F jS, Y', strtotime($questionnaire->updated_at)) }}</td>
                                            <td>
                                                <label class="switch">
                                                    <input data-id="{{ $questionnaire->id }}" class="toggle-class"
                                                        type="checkbox" data-onstyle="success" data-offstyle="danger"
                                                        data-toggle="toggle" data-on="Active" data-off="InActive"
                                                        {{ $questionnaire->status ? 'checked' : '' }}>
                                                    <span class="slider round"></span>
                                                </label>
                                            </td>
                                            <td>
                                                <a target="_blank" href="javascript:void(0)" class="editeType"
                                                    data-id="{{ $questionnaire->id }}" data-name="{{ $questionnaire->name }}" data-modal="#edit-questionnaire-modal"><img
                                                        src="{{ asset('ui_assets/img/icons/vuesax-bulk-document-text.svg') }}"></a>
                                                <button class="client-act-btn deleteType" data-id="{{ $questionnaire->id }}"  data-name="{{ $questionnaire->name }}" 
                ...
                <form action="{{ route('admin.questionnaire.delete') }}" method="POST">
                    ...
                    <div class="modal-caption">
                        ...
                        <p class="mc-subtitle">Are you sure you wish to delete this
                            Questionnaire "<strong id="delete_questionnaire_name"></strong>"? This action will&nbsp;remove the
    ...
    $(document).ready(function() {
        ...
        $('.toggle-class').change(function() {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "{{ route('admin.questionnaire.changeStatus') }}",
                data: {
                    'status': status,
