                        ...
                        <div class="column">
                            <div class="question-block">
                                <div class="block-top">
                                    ...
                                    <h3 class="page-user">
                                    User : {{ project.pages_information[page_index].users[current_index].user }} - Information Displayed
                                    </h3>
                        ...
                        <div class="column">
                            <div class="question-block">
                                <div class="block-top">
                                    ...
                                    <h3 class="page-user">
                                        User : {{ project.pages_information[page_index].users[current_index].user }} -  Perfomable Actions
                                    </h3>
                                    <div class="block-note">
                                        <p>
                                            Please define what information is
...
export default {
    ...
    created() {
        ...
        //     if(el.user && (el.information[0].text != '' || el.actions[0].text != '') )
        //     {
        //         const element = el.user;
        //         if (values.indexOf(element) !== -1) {
        //             let index = values.indexOf(element)
        //             let count = 1
        //             // _this.project.pages_information[_this.page_index].users[index].information = _this.project.pages_information[_this.page_index].users[index].information
        //             // _this.project.pages_information[_this.page_index].users[index].actions = _this.project.pages_information[_this.page_index].users[index].actions
    ...
    methods: {
        ...
        saveSixth() {
            if(this.project.pages_information[this.page_index].users.length-1 >= this.current_index + 1 ) {
                EventBus.$emit('repeat-sixth-step', this.current_index, this.duplicate)    
            }
                ...
                if(this.page_index < (this.project.number_of_pages - 1)) 
                {
                    ...
                    EventBus.$emit('repeat-sixth-step-for-next-page', this.page_index)
                }
                else
                {
                    EventBus.$emit("sixth-step-submitted");    
                }
            }
            else
            {
        ...
        skipUser() {
            if(this.project.pages_information[this.page_index].users.length-1 >= this.current_index + 1 ) {
                EventBus.$emit('skip-user', this.current_index)    
            }
            else if(this.current_index == (this.project.pages_information[this.page_index].users.length - 1)) {
                if(this.page_index < (this.project.number_of_pages - 1)) 
                {
                    EventBus.$emit('skip-page', this.page_index)
                }
                else {
        ...
        goToFifth() {
            if( this.current_index > 0 && (this.project.pages_information[this.page_index].users.length-1) >= (this.current_index) ) {
                EventBus.$emit('previous-sixth-step', this.current_index)    
            }
            else if(this.page_index > 0 && this.page_index <= this.project.pages_information.length)
            {
                EventBus.$emit('previous-sixth-step-for-page', this.page_index)    
            }
            else
            {
                EventBus.$emit("goToFifthForm-clicked");
            }
        },
